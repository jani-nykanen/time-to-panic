import { ProgramEvent, InputState, Assets } from "../core/interface.js";
...
export class Menu {
    ...
    public draw(canvas : Canvas, assets : Assets,
        ...
        const h : number = (this.height + 0.5)*yoff;
        ...
        for (let i : number = 0; i < this.buttons.length; ++ i) {
            ...
            const xoff : number = Number(i == this.cursorPos)*(charDim + CHAR_X_OFF)*1.5;

            // Item text
            canvas.drawText(font, this.buttons[i].getText(), 
                dx + xoff + SIDE_OFFSET, dy + SIDE_OFFSET + i*yoff, CHAR_X_OFF, 0);
            // Cursor
            if (i == this.cursorPos) {
