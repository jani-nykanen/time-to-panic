...
export class ConfirmationBox {
    ...
    public draw(canvas : Canvas, assets : Assets,
        ...
        yoff : number = -2, menuYoff : number = -2,
        ...
        const w : number = (this.width + 1)*(charDim + CHAR_X_OFF);
        const h : number = (this.height + 1)*(yoff) + (charDim + menuYoff)*2;
        ...
        for (let i : number = 0; i < this.message.length; ++ i) {
            ...
            canvas.drawText(font, this.message[i], dx + SIDE_OFFSET, dy + SIDE_OFFSET + i*yoff, CHAR_X_OFF, 0);
        }

        const menuY : number = (dy + h - canvas.height/2) - (charDim + menuYoff);
